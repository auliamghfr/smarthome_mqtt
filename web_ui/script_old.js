// API Configuration (HTTP Polling instead of WebSocket)
// Using MQTT Proxy Server at port 5000
    ...
    fetch(`${API_URL}/api/data`)
        ...
        .then(data => {
            if (data) {
                ...
                if (data.motion) {
                    handleMotion(data.motion);
                }
                if (data.lamp_status) {
                    handleLampStatus(data.lamp_status);
                }
            }
        })
        .catch(error => {
...
function handleTemperature(data) {
    ...
    // Update gauge
    const gaugeElement = document.querySelector('[data-type="temperature-gauge"]');
    if (gaugeElement) {
        gaugeElement.innerHTML = `${temp.toFixed(1)}<span>${unit}</span>`;
    }
    
    // Update chart
    addChartData(temp, 'temperature');
    
    // Add log
    addLogEntry('🌡️ Temperature', `${temp}${unit}`, 'success');
}

function handleMotion(data) {
    if (!data) return;
    
    const status = data.status || (data.value === 1 ? 'motion detected' : 'no motion');
    const motionValue = data.value === 1 ? 1 : 0;
    
    // Update motion indicator
    const motionElement = document.querySelector('[data-type="motion-status"]');
    if (motionElement) {
        motionElement.innerHTML = status;
        motionElement.className = motionValue === 1 ? 'motion-detected' : 'motion-clear';
    }
    
    // Update chart
    addChartData(motionValue, 'motion');
    
    // Add log
    const logMsg = motionValue === 1 ? '🚶 Motion Detected' : '🚫 No Motion';
    addLogEntry(logMsg, status, motionValue === 1 ? 'warning' : 'info');
}

function handleLampStatus(data) {
    if (!data) return;
    
    const state = data.state || 'OFF';
    const isOn = state.toUpperCase() === 'ON';
    
    // Update lamp indicator
    const lampElement = document.querySelector('[data-type="lamp-status"]');
    if (lampElement) {
        lampElement.innerHTML = isOn ? '🟢 ON' : '🔴 OFF';
        lampElement.className = isOn ? 'lamp-on' : 'lamp-off';
    }
    
    // Update switch
    const switchElement = document.querySelector('[data-type="lamp-switch"]');
    if (switchElement) {
        switchElement.checked = isOn;
    }
    
    // Add log
    addLogEntry('💡 Lamp', state, isOn ? 'success' : 'info');
}
...
function publishCommand(topic, message) {
    // Send lamp command via HTTP API
    ...
    .then(data => {
        if (data.status === 'success') {
            console.log(`✅ Lamp command sent: ${command}`);
            addLogEntry('💡 Command Sent', `Lamp set to ${command}`, 'success');
        } else {
            console.error('❌ Failed to send lamp command:', data.message);
        }
    })
    .catch(error => {
        console.error('❌ Error sending lamp command:', error);
    });
}
                new Date().toLocaleTimeString(),
                'Dashboard',
